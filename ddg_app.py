# ddg_app.py (FINAL VERSION WITH ALL REQUESTED CHANGES)

import streamlit as st
import urllib.parse 

# --- 1. TITLE AND PHENOMENON SELECTION (NGSS PEs) ---
# CHANGE 1: Renaming the title
...
# CHANGE 2: Expanded the list of phenomena for increased utility
pe_id = st.selectbox(
    ...
    index=0, # Default to "Select a Phenomenon"
...
# Initialize placeholders for parameters needed by the backend
L_param = 0.0
k_param = 0.0
t_range = 0
mass_const = 0.0
sigma_noise = 0.0 # Will be overridden by Section 3 slider
...
# --- NEW LOGIC FOR EXPANDED PHENOMENA ---
    ...
    # Mapping to standard DDG inputs (must be handled correctly in Colab)
    L_param = E_in
    k_param = Efficiency
    t_range = Num_Trials
    mass_const = 0.0
    ...
    # Mapping to standard DDG inputs (L represents Optimum Temp, k represents Stability Factor)
    ...
    # Mapping to standard DDG inputs
...
# This slider is applied universally regardless of the PE selected
...
# CRITICAL: These Entry IDs were extracted from your pre-filled form URL
...
if st.button("Generate Synthetic Data"):
    if pe_id == "Select a Phenomenon":
        ...
        # We submit the PE ID, the main DCI parameter (L_param), and the Noise (Sigma)
        url_params = {
            PE_ID_ENTRY: pe_id,       
